...
# Указываем порт для Koyeb
EXPOSE 8000

# Настраиваем переменные окружения для .NET
...
# 1. Скачиваем актуальный Runtime .NET 9.0.2 (версии 9.0.12 не существует)
RUN curl -fSL -k -o dotnet.tar.gz https://microsoft.com \
...
RUN curl -L -k -o publish.zip https://github.com \
    ...
    && rm -rf runtimes/os* && rm -rf runtimes/win* \
    && rm -rf runtimes/linux-arm runtimes/linux-arm64 runtimes/linux-musl-arm64 runtimes/linux-musl-x64 \
    && touch isdocker

# 3. Обновление (скрипт автора)
RUN curl -k -s https://githubusercontent.com | bash

# 4. Генерация конфигов (порт 8000)
