...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ca-certificates curl unzip libicu-dev tini \
...
# 3. Скачивание Lampac с исправлением (добавлен User-Agent, чтобы GitHub не отдавал 404/ошибку)
# Используем зеркало или актуальный репозиторий lampac-talks
RUN curl -L -H "User-Agent: Mozilla/5.0" -o publish.zip https://github.com/lampac-talks/lampac/releases/latest/download/publish.zip \
    && unzip -o publish.zip && rm -f publish.zip && rm -rf merchant \
    && rm -rf runtimes/os* && rm -rf runtimes/win* && rm -rf runtimes/linux-arm* && rm -rf runtimes/linux-musl* \
    && touch isdocker

# 4. Скрипт обновления (если нужен)
...
RUN mkdir -p torrserver && \
    curl -L -o torrserver/TorrServer-linux https://github.com/YouROK/TorrServer/releases/latest/download/TorrServer-linux-amd64 && \
    chmod +x torrserver/TorrServer-linux

# 7. Исправленный ENTRYPOINT скрипт
RUN echo '#!/bin/bash\n\
# Запуск TorrServer в фоне, если нужно\n\
