...
# Исправлен формат init.conf (оставлен твой оригинал)
...
# Добавлены ENV и правильные JSON скобки для JacRed
ENV JACRED_HOST=https://jackett.xyz
ENV TORRSERVER_HOST=http://127.0.0.1:8090
ENV SEARCH_TORRENT=true

RUN echo "{\"typesearch\":\"jacred\",\"host\":\"$JACRED_HOST\"}" > /home/module/JacRed.conf
...
# Создаем скрипт запуска для одновременной работы TorrServer и Lampac
RUN echo '#!/bin/bash\n/home/torrserver/TorrServer-linux -p 8090 &\n/usr/share/dotnet/dotnet /home/Lampac.dll' > /home/entrypoint.sh && chmod +x /home/entrypoint.sh
